...
while True:
	...
	#hsv = cv2.Mat
# Blue 
    colorFrame = cv2.cvtColor(img,cv2.COLOR_BGR2HSV);
    LowValue = numpy.array([110,50,50]);
    HighValue = numpy.array([130,255,255]);

    masking = cv2.inRange(colorFrame, LowValue, HighValue);

    blueFrame2bit = cv2.bitwise_and(img,img,mask=masking);

    cv2.imshow('blueFrame2bit',blueFrame2bit);

    cv2.waitKey(2);

# YELLOW
    colorFrame = cv2.cvtColor(img,cv2.COLOR_BGR2HSV);
    LowValue = numpy.array([20,100,100]);
    HighValue = numpy.array([30,255,255]);

    masking = cv2.inRange(colorFrame, LowValue, HighValue);

    yellowFrame2bit = cv2.bitwise_and(img,img,mask=masking);

    cv2.imshow('yellowFrame2bit',yellowFrame2bit);
    height, width = colorFrame.shape[:2]
